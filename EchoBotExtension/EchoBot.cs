			...
			CancellationToken cancellationToken = default(CancellationToken))
		{
			...
			if (turnContext.Activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(turnContext.Activity.Text))
			{
				...
				else
				{
					...
					if (ParseUtils.TryParseArithmeticOperation(
						    turnContext.Activity.Text, out var num1, out var num2, out var result, out var operation))
					{
						await turnContext.SendActivityAsync($"Первое число -> '{num1}'",
							cancellationToken: cancellationToken);
						await turnContext.SendActivityAsync($"Второе число -> '{num2}'",
							cancellationToken: cancellationToken);
						await turnContext.SendActivityAsync($"Арифметический оператор -> '{operation}'",
							cancellationToken: cancellationToken);
						await turnContext.SendActivityAsync($"Результат = '{result}'",
							cancellationToken: cancellationToken);
					}
					else
					{
